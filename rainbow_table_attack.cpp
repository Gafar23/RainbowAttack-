...
int main(int argc, char *argv[]) {
    ...
    if (argc != 5) {
        ...
        std::cerr << std::endl;
        std::cerr << "Usage : \"rainbow_table_attack nb_of_reductions pwd_size rainbow_table_path hash_to_crack_path, where" << std::endl
    ...
    for (auto e : is_pwd_found) {
        std::cout << "pwd n*" << count << " found : " << std::boolalpha << e << std::endl;
        count++;
    }
