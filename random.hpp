...
template <class T>
T random(T min = std::numeric_limits<T>::min(), T max = std::numeric_limits<T>::max(), bool reproducible = false) {
    if (max < min)
...
std::shared_ptr<std::vector<T>> sparse_distinct_random(int n, T min = std::numeric_limits<T>::min(),
    ...
    for (int i = 0; i < n; i++) {
        ...
        while (std::find(out->begin(), out->end(), nbr) != out->end())  
            nbr = rainbow::random(min, max, reproducible);              
